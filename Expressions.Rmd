...
logical_abbr_rec <- function(x) {
  switch_expr(x,
    ...
    # Recursive cases
    call = , #If there is a match then that element is evaluated unless it is missing, in which case the next non-missing element is evaluated
    pairlist = purrr::some(x, logical_abbr_rec)
  )
}

logical_abbr(mean(x, na.rm = T)) # matches as call, but then 'pairlist' line is excuted. Remember, a call is a list
logical_abbr(function(x, na.rm = T) FALSE)
```

### Finding all variables created by assignment
The next task, listing all variables created by assignment, is a little more complicated. We’ll start simply, and then make the function progressively more rigorous. We start by looking at the AST for assignment:

```{r}
ast(x <- 10)
```

Assignment is a call where the first element is the symbol `<-`, the second is name of variable, and the third is the value to be assigned. Next, we need to decide what data structure we’re going to use for the results. Here I think it will be easiest it we return a character vector.
...
find_assign(a <- 1) # a is a symbol
find_assign("a" <- 1) # is a character. Note that "a" <- 1 is valid syntax
find_assign({a <- 1; b <- 2})

# some ways of passing a pairlist directly
pairlist(x = expr(a <- 1))
...
# Note that is_symbol(x[[2]]) means that assigments of the form "a" <- 1 are not captured
# Should also check for a character
# Why check at all? Because of replacement functions (see exercises)
```
...
2. `logical_abbr()` works with expressions. It currently fails when you give it a function. Why not? How could you modify `logical_abbr()` to make it work? What components of a function will you need to recurse over?
    ...
    logical_abbr(!!f)
    ```
**A**: We need to modify `expr_type(x)` and `logical_abbr_rec()` to handle functions.
    ```{r}
    expr_type <- function(x) {
      if (rlang::is_syntactic_literal(x)) {
        "constant"
    ...
    logical_abbr_rec <- function(x) {
      switch_expr(x,
        ...
        # function
        fun = if (logical_abbr_rec(formals(x))) TRUE else logical_abbr_rec(body(x))
...
**A**: First let's look at the AST:
    ```{r}
    lobstr::ast(names(x) <- y)
    expr(names(x) <- y)[[2]]
    typeof(expr(names(x) <- y)[[2]])
    ```
Note that the second element is a call, namely `names`. Hence we need to update `find_assign_call` as follows:
    ```{r}
    find_assign_call <- function(x) {
      
      # added is_character to handle assigments of form "a" <- 1 
      if (is_call(x, "<-") && (is_symbol(x[[2]]) || is_character(x[[2]]))) {
        lhs <- as_string(x[[2]])
        children <- as.list(x)[-1]
        
      # updated to handle replacement functions
...
**A**: 
    ...
    # changed
    find_function <- function(x, FUN) unique(find_function_rec(enexpr(x), enexpr(FUN)))
    
    find_function_rec <- function(x, FUN) {
      switch_expr(x,
                  # Base cases
                  constant = ,
                  symbol = character(),
                  ...
                  # Recursive cases
                  pairlist = flat_map_chr(x, find_funtion_rec, FUN),
                  call = find_function_call(x, FUN)
      )
    }
    
    find_function_call <- function(x, FUN) {
      if (is_call(x, FUN)) {
        lhs <- expr_text(x)
        children <- as.list(x)[-1]
      } else {
      ...
      c(lhs, flat_map_chr(children, find_function_rec, FUN))
    }
    
    # examples
    find_function(a <- 1, `<-`)
    find_function(a <- b <- 1, `<-`)
    find_function(mean(1:3), mean)
    find_function(mean(mean(1:3)), mean)
    ```
